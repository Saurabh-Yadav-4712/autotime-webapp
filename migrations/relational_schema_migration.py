...
def run_migration():
    ...
    for TeacherModel in [Teacher, Course, Subject, Timetable, Settings]:
        ...
        for r in records:
            if not getattr(r, "institute_code", None):
                continue
            if r.institute_code not in inst_map:
                raise MigrationIntegrityError(
                    f"Unknown institute_code {r.institute_code} on {TeacherModel.__name__} ID {r.id}"
                )
    ...
    for s in subjects:
        ...
        if key not in teacher_map:
            raise MigrationIntegrityError(
                f"Unknown teacher_id '{s.teacher_id}' for institute {s.institute_id} on Subject ID {s.id}"
            )
    ...
    for s in subjects:
        ...
        for cid in class_ids:
            ...
            if key not in course_map:
                raise MigrationIntegrityError(
                    f"Unknown class_id '{cid}' for institute {s.institute_id} on Subject ID {s.id}"
                )
    ...
    for tt in timetables:
        ...
        if not subject_match:
            raise MigrationIntegrityError(
                f"Unknown subject '{tt.subject_name}' on Timetable ID {tt.id}"
            )
        ...
        if not getattr(tt, "is_proxy", False) and subject_match:
            ...
            if key_teacher not in teacher_name_map:
                raise MigrationIntegrityError(
                    f"Unknown teacher_name '{tt.teacher_name}' for institute {tt.institute_id} on Timetable ID {tt.id}"
                )
            ids = teacher_name_map[key_teacher]
            if len(ids) > 1:
                raise MigrationIntegrityError(
                    f"Ambiguous teacher_name '{tt.teacher_name}' for institute {tt.institute_id}. Multiple Teacher IDs: {ids}"
                )
...
def verify_migration():
    ...
    # Check orphans
    tt_nulls = Timetable.query.filter(
        (Timetable.institute_id == None)
        | (Timetable.course_id_fk == None)
        | (Timetable.teacher_id_fk == None)
        | (Timetable.subject_id_fk == None)
    ).count()
    if tt_nulls > 0:
