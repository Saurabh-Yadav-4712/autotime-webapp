...
def run_gap_test(schedule_dict, expected_internal_gaps):
    ...
    assert actual_internal_gaps == expected_internal_gaps, f"Expected {expected_internal_gaps}, Got {actual_internal_gaps}"
    return monday_slots
...
def test_screenshot_practical_case():
    ...
    for s in slots:
        assert s["status"] == "occupied", f"Slot {s['start_time']} should be occupied, got {s['status']}"
