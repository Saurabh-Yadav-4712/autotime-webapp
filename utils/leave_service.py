...
def cancel_leave(leave_id, actor_name):
    """
    ...
    try:
        if leave.status == 'Approved':
            ...
            for proxy in proxies:
                proxy_teacher = Teacher.query.filter_by(institute_code=proxy.institute_code, name=proxy.teacher_name).first()
                if proxy_teacher:
                    msg = f"CANCELLATION: Your proxy assignment for {proxy.subject_name} on {proxy.specific_date} has been cancelled."
                    db.session.add(Notification(institute_code=proxy.institute_code, user_type='teacher', user_id=proxy_teacher.teacher_id, message=msg))
        ...
        # Notify admin
        teacher = Teacher.query.filter_by(teacher_id=leave.teacher_id, institute_code=leave.institute_code).first()
        ...
        db.session.add(Notification(institute_code=leave.institute_code, user_type='admin', message=admin_msg))
...
def allocate_proxy_for_leave(leave):
    ...
    teacher = next((t for t in all_teachers if t.teacher_id == leave.teacher_id), None)
    ...
    concurrent_leaves = TeacherLeave.query.filter_by(institute_code=inst_code, date=target_date, status='Approved').all()
    ...
    for lec in missed_lectures:
        ...
        for proxy_t in all_teachers:
            ...
            proxy_days = [d.strip() for d in (proxy_t.available_days or "").split(',')] if proxy_t.available_days else ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']
            if day_name not in proxy_days: continue
            
            # Check busy status
            is_busy = Timetable.query.filter_by(
                institute_code=inst_code, 
                teacher_name=proxy_t.name, 
                day_name=day_name, 
                start_time=st
            ).filter((Timetable.specific_date == None) | (Timetable.specific_date == target_date)).first()
            
            if not is_busy:
                score = 0
                if lec.subject_name in teacher_subjects.get(proxy_t.name, set()): score += 5
                if lec.class_id in teacher_classes.get(proxy_t.name, set()): score += 3
                if proxy_t.departments == class_dept: score += 1
