...
class GlobalState:
    ...
    teacher_busy: Dict[str, Dict[str, Set[int]]] = field(default_factory=dict) # teacher_id -> day -> set of slot idx
    class_busy: Dict[str, Dict[str, Set[int]]] = field(default_factory=dict) # class_id -> day -> set of slot idx
    ...
    def unassign(self, teacher_id: str, classes: List[str], day: str, slot_idx: int, duration: int):
        ...
        if teacher_id:
            self.teacher_hours[teacher_id] = max(0, self.teacher_hours.get(teacher_id, 0) - duration)
