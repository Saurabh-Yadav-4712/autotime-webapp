...
from models import db, Institute, Teacher, Course, Subject, Settings, Timetable, SubjectCourse, TeacherLeave
...
def seed_db():
    i1 = Institute(name="Inst 1", institute_code="I1", admin_username="a1", admin_email="a1@x", admin_password="1")
    i2 = Institute(name="Inst 2", institute_code="I2", admin_username="a2", admin_email="a2@x", admin_password="2")
    db.session.add_all([i1, i2])
    db.session.commit() # commit so they get IDs

    t1 = Teacher(institute_code="I1", teacher_id="T1", name="Alice", email="t1@x", departments="CS", available_days="Mon,Tue", max_hours=20)
    t2 = Teacher(institute_code="I1", teacher_id="T2", name="Bob", email="t2@x", departments="CS", available_days="Mon", max_hours=20)
    ...
    s1 = Subject(institute_code="I1", subject_code="S1", subject_name="Math", class_id="CS1", teacher_id="T1", required_hours=2)
    s2 = Subject(institute_code="I1", subject_code="S2", subject_name="Physics", class_id="CS1,CS2", teacher_id="T2", required_hours=3) # common
    db.session.add_all([s1, s2])

    tt1 = Timetable(institute_code="I1", class_id="CS1", day_name="Mon", start_time="09", end_time="10", subject_name="Physics", teacher_name="Bob")
    tt2 = Timetable(institute_code="I1", class_id="CS2", day_name="Mon", start_time="09", end_time="10", subject_name="Physics", teacher_name="Bob")
    db.session.add_all([tt1, tt2])
    
    lv1 = TeacherLeave(institute_code="I1", teacher_id="T1", date=__import__("datetime").date(2026, 8, 26), status="Approved")
    db.session.add(lv1)
